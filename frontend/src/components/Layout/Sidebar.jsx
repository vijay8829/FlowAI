...
import {
  Zap, LayoutGrid, Activity, Plus, ChevronRight, Command,
} from 'lucide-react';
...
export default function Sidebar({ onCommandPalette }) {
  ...
  return (
    <>
      {/* ── Desktop sidebar ───────────────────────────── */}
      <aside className="sidebar-mesh hidden md:flex w-56 flex-col border-r border-white/[0.06] shrink-0 relative overflow-hidden">
        {/* Logo */}
            ...
            return (
              <button key={id} onClick={() => setView(id)}
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-150 group ${
                  active ? 'bg-indigo-500/15 text-white shadow-glow-xs' : 'text-slate-500 hover:text-slate-300 hover:bg-white/[0.05]'
                }`}
              >
        ...
        <div className="px-3 mt-5 mb-2">
          ...
          {creating && (
            <form onSubmit={handleCreate} className="mb-2 anim-fade-down">
              <input autoFocus className="input input-sm w-full text-[12px]" placeholder="Workflow name…"
          ...
          <div className="space-y-0.5 max-h-48 overflow-y-auto">
            {workflows.slice(0, 8).map(wf => (
              <button key={wf.id} onClick={() => useWorkflowStore.getState().openWorkflow(wf.id)}
                className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-left transition-all group ${
                  useWorkflowStore.getState().activeWorkflow?.id === wf.id && view === 'builder'
                    ? 'bg-white/[0.07] text-slate-200' : 'text-slate-600 hover:text-slate-300 hover:bg-white/[0.04]'
                }`}
              >
      ...
      {/* ── Mobile bottom nav ─────────────────────────── */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around border-t border-white/[0.08] bg-panel/95 backdrop-blur-md safe-bottom">
        ...
        <button onClick={onCommandPalette}
          className="flex flex-col items-center gap-1 px-6 py-3 text-slate-600 hover:text-slate-400 transition-all"
        >
